...
import { ensureBookingTool } from "@/lib/bookingTool";
...
Rules:
- message: a short, friendly reply confirming what you built, shown to the user in the builder chat (e.g. "Done — I've set up an agent that..."). This is never spoken by the voice agent.
...
export async function POST(req: Request) {
  try {
    ...
    const bookMeetingToolId = await ensureBookingTool();
    const payload = buildAssistantPayload(config, bookMeetingToolId);
